...
What is the relationship of song rank (`pos`) and the top 10 artists (`artist_name`)?
```{r}
...
What is the relationship between playlist (`playlist_name`) and pos?
```{r}
ggplot(data = spotify_all) +
  geom_boxplot(mapping = aes(x = playlist_name, y = pos))
```
